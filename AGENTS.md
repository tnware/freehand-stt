...
- Maintain task-oriented product documentation when setup, usage, privacy, or troubleshooting changes. Routine visual and internal refactors need no documentation entry. Update `README.md` only when its product summary, requirements, safety promises, or public status changes.
- Keep contributor documentation focused on actionable setup, build, release, generation, and safety procedures. Record implementation decisions, validation results, and delivery state in the relevant issue or pull request.

- Before opening a pull request, reconcile its linked issue and affected product documentation against what actually shipped. Do not leave completed work checked as future work or describe implemented features as post-MVP.
