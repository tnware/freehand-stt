...
- Keep one Go module. Native adapters live by capability under `internal/platform/{audio,keyboard,input,overlay,startup}`, with OS variants and native bridge files beside their owner. Managed-runtime artifact and process mechanics live in its private `internal/artifact` and `internal/process` leaf packages; inventory, worker state, publication, and output access remain under `internal/managedruntime`. Group same-owner methods in descriptive files, keep tests beside implementations, and do not export locks or mutable state merely to create a subpackage.
...
- Keep affected product documentation accurate in the same branch when user-visible behavior changes: setup, workflows, settings, privacy, and troubleshooting. Routine visual or internal refactors do not need a documentation entry unless they change how someone uses the product. Record implementation details, validation results, and delivery state in the relevant issue or pull request. Do not append per-change summaries or test inventories to architecture or testing documents. Maintain contributor references only when their existing guidance would otherwise become incorrect; do not require an architecture/testing documentation update for every feature or fix. Update `README.md` only when the product summary, requirements, principal capabilities, safety promises, or public project status changes.
