...
func TestMacBuildVersionRejectsUnsupportedReleases(t *testing.T) {
	base := "info:\n  productName: Freehand\n  productIdentifier: io.github.tnware.freehand\n  version: "
	for _, version := range []string{"0.1.0-preview.1", "0.1.0-alpha.test.1", "0.1.0-alpha.0", "0.1.0-beta.65536", "1.2.3.4"} {
		if _, err := parseMacInfo([]byte(base + version + "\n")); err == nil {
...
func TestCommittedMacMetadataSynchronized(t *testing.T) {
	...
	for _, dev := range []bool{false, true} {
		...
		if !sameReleaseAsset(actual, expected) {
...
`)
	...
	for _, dev := range []bool{false, true} {
		...
		for k, want := range map[string]string{"CFBundleDisplayName": "Freehand & Friends", "CFBundleName": "Freehand & Friends", "CFBundleIdentifier": id, "CFBundleExecutable": "freehand", "CFBundleShortVersionString": "0.1.0", "CFBundleVersion": "0.1.5", "FreehandReleaseVersion": "0.1.0-alpha.5", "LSMinimumSystemVersion": "13.0"} {
