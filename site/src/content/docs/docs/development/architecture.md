...
recording-start action. macOS left-click instead opens an app-owned, status-item
anchored nonactivating `tray-popover` NSPanel; right-click retains the menu.
...
external-target override or application activation. Main and Settings navigation
also hides the panel. The runtime controller does not synthesize or repurpose
