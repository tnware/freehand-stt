...
per-user file permissions. `freehand.db`, backups, and the separate
`window-state.json` live there. Credentials remain in Keychain. The Windows
paths and ACL details below apply only to Windows.

On Windows, the database is `%LOCALAPPDATA%\Freehand\freehand.db`, with a
...
## Initialization, upgrade, and recovery

With no `freehand.db`, initialization builds safe defaults and an empty connection
catalog in a private temporary database, validates it, and publishes it only when
...
old database and any journal/WAL sidecars in `freehand-recovery-*`, and publishes
...
keys again. Explicit reset starts fresh without retaining connection or credential
references: users reconfigure connections and re-enter keys. Native credential
records are not automatically purged. Do not log SQL, values, native account identifiers, or file
paths when handling these failures.

## Credential consistency

For a key replacement, persist cleanup intent, write a new native account named
`freehand-<purpose>-<randomhex>`, and stage its reference. Static alpha and old
`sqlite-*` account references are not accepted. A single SQLite transaction commits all settings
...
Run real temporary SQLite fixtures on Windows and macOS, including process exit during
...
Fresh initialization starts with no saved connections. Connections own reusable
endpoint and authentication details with explicit supported uses; models and
options belong to features. The serialized settings owner validates bounded,
non-conflicting names across the library.
...
same transaction. Only references in the current database participate in this
lifecycle; legacy alpha accounts are not adopted or cleaned up.
...
Model-profile selection is feature-owned. Generated queries persist the active
profiles through the same settings transaction. Unknown profile IDs enter recovery
instead of silently falling back to Generic.

## Remembered model preferences

The baseline has a STRICT remembered-model table with an explicit
