...
- Use the quick controls to select a connection and model, toggle cleanup, switch between Custom instruction and S1-mini, and exercise the trained S1-mini controls. Valid quick changes must save immediately; failed saves must leave backend-applied configuration unchanged. Endpoint, authentication, and credential edits belong to Connections inside Settings. In full Cleanup settings, verify custom-instruction validation/reset/persistence and the read-only S1 instruction/control-line preview. Confirm changed selections invalidate stale metadata, quick saves cannot race or apply an unrelated unsaved Settings draft, and Main quick controls remain disabled while Settings owns configuration.
