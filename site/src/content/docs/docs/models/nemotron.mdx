...
for Voice, audio files, or both. The recognition list has these limits:
...
Vocabulary guides recognition; use the separate Cleanup stage for rewrite instructions.

<details>
<summary>How language evidence affects cleanup</summary>

Freehand removes the terminal language tag from displayed text and retains it
as language evidence alongside structured metadata. Realtime finals retain
languages across the entire recording: a later English turn does not erase
earlier non-English evidence for cleanup.

</details>

:::note[Live captions are a preview]
