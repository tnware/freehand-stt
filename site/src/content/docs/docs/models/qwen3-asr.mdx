...
Have vLLM **0.28.0** running the **Qwen3-ASR 1.7B** checkpoint with the realtime
architecture below. If you need to start it, follow [Run the server](#run-the-server)
first, then return here with its base URL and authentication details.
...
5. Choose **Save** to apply the workflow settings.
...
:::note[Language hints have a narrower list]
The model card lists 30 languages and Chinese dialect recognition. vLLM 0.28.0
supports only 28 matching explicit hints: use automatic detection for Cantonese
and Filipino; do not substitute a Tagalog hint.
:::
...
:::note[Only final text is delivered]
Freehand displays provisional text without Qwen's structured language headers.
Stopping asks the server to finalize within 30 seconds. Disconnects, cancellation,
and missing finals discard the preview.
:::
...
```sh title="Serve Qwen3-ASR with realtime enabled"
...
<CardGrid>
  <LinkCard title="Use live transcription" href="../../guides/live-transcription/" description="Understand captions, recording, and final delivery." />
  <LinkCard title="Add recognition vocabulary" href="../../guides/vocabulary/" description="Supply names and terminology for completed transcription." />
</CardGrid>
