...
// Static bounded first-failure category only; never AX data or identity.
// Read before the next capture/validate/send or reset. Cleanup preserves it;
// successful operations return an empty category. The returned literal is static.
const char *fh_input_rejection(fh_input_owner *owner);
