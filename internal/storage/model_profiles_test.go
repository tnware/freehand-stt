...
func TestVersionFiveModelProfileUpgradePreservesCleanupAndConnections(t *testing.T) {
	...
	if _, err := s.db.Exec(`DROP TABLE realtime_settings; DELETE FROM credential_refs WHERE purpose='realtime'; DROP TABLE remembered_models; ALTER TABLE transcription_settings DROP COLUMN model_profile; ALTER TABLE speech_settings DROP COLUMN model_profile; DELETE FROM goose_db_version WHERE version_id>=6;`); err != nil {
		t.Fatal(err)
	}
	s = reopen(t, s)
	after := loadStore(t, s)
