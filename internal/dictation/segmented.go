...
func transcribePCM(ctx context.Context, client *inference.Client, cfg config.Settings, key string, pcm []byte) (inference.TranscriptionResult, error) {
	...
	defer cancel()
	return client.WithCompatibility(cfg.CompatibilityProfile).WithModelProfile(cfg.ModelProfile).WithTranscriptionOptions(cfg.TranscriptionOptions.Inference()).Transcribe(requestCtx, cfg.BaseURL, cfg.Model, cfg.Language, key, cfg.Headers, wav)
}
