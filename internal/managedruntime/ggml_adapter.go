...
type ggmlAdapter struct {
	...
	recipe         ggmlProvider
	launch         func(context.Context, string, []string, string, []string) (processHandle, error)
...
func (a *ggmlAdapter) installedBackend(ctx context.Context) (string, error) {
	if err := artifact.Recover(a.root); err != nil {
		return "", err
	}
	...
	if err := artifact.Verify(ctx, filepath.Join(a.root, "runtime"), b); err != nil {
		return "", err
	}
...
func (a *ggmlAdapter) Inspect(ctx context.Context) (string, []Model, error) {
	...
	for i := range models {
		...
		models[i].Installed = artifact.VerifyFile(ctx, s.path(a.root), s.size, s.sha256) == nil
		if ctx.Err() != nil {
			return "", nil, ctx.Err()
		}
	}
...
func (a *ggmlAdapter) InstallBackend(ctx context.Context, backend string, progress func(float64)) (string, error) {
	...
	if err := artifact.Install(ctx, a.root, b, client, progress); err != nil {
		return "", err
	}
	return backend, nil
}
...
func acquireModel(ctx context.Context, root string, s modelSpec, client *http.Client, progress func(AcquisitionProgress)) error {
	...
	if err := artifact.CheckRoot(root); err != nil {
		return err
	}
	if err := artifact.VerifyFile(ctx, s.path(root), s.size, s.sha256); err == nil {
		return nil
	} else if !os.IsNotExist(err) {
		return err
	}
	...
	n, err := io.Copy(&acquisitionWriter{Writer: f, report: &report, total: s.size}, io.LimitReader(artifact.ContextReader{Context: bounded, Reader: res.Body}, s.size+1))
	ce := f.Close()
	if err != nil {
		return err
	}
	...
	if err = artifact.VerifyFile(bounded, stage, s.size, s.sha256); err != nil {
		return err
	}
	if err = artifact.CheckRoot(root); err != nil {
		return err
...
func (a *ggmlAdapter) waitReady(ctx context.Context, p processHandle, base string, port int, model string) error {
...
func (a *ggmlAdapter) Start(ctx context.Context, id string) (processHandle, Endpoint, error) {
	s, ok := a.recipe.specs[id]
	if !ok {
	...
	if err := artifact.VerifyFile(ctx, s.path(a.root), s.size, s.sha256); err != nil {
		return nil, Endpoint{}, err
	}
