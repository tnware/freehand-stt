...
CPU and CUDA argv now replace `--log-disable` with `--log-verbosity 3 --log-colors off`.
...
in private stderr capture, no ANSI color escapes, and no new files beneath the
temporary installation. It failed with `--log-disable` and passed with the new
policy. It performs no model download, load, inference, or user-runtime mutation.
