...
func newAdapter(root string) *nemoAdapter {
	return &nemoAdapter{root: root, launch: launchOwned, listenerOwner: process.OwnsListener}
}
...
func (a *nemoAdapter) installedBackend(ctx context.Context) (string, error) {
	...
	if err = artifact.Verify(ctx, filepath.Join(a.root, "runtime"), selected.Bundle); err != nil {
		return "", err
	}
...
func (a *nemoAdapter) chooseAsset(ctx context.Context) artifact.Asset {
...
func (a *nemoAdapter) Install(ctx context.Context, progress func(float64)) (string, error) {
	...
	if err := artifact.Install(ctx, a.root, recipe.Bundle, client, progress); err != nil {
		return "", err
	}
	if err := artifact.Verify(ctx, filepath.Join(a.root, "runtime"), recipe.Bundle); err != nil {
		return "", err
	}
...
func (a *nemoAdapter) waitReady(ctx context.Context, p processHandle, base string, port int) (string, error) {
	model, _, err := a.waitReadyModels(ctx, p, base, port, false)
	return model, err
}

func (a *nemoAdapter) waitReadyModels(ctx context.Context, p processHandle, base string, port int, speech bool) (string, string, error) {
...
func (a *nemoAdapter) Start(ctx context.Context, id string) (processHandle, Endpoint, error) {
	return a.StartModels(ctx, id, "")
}

func (a *nemoAdapter) StartModels(ctx context.Context, id, speechID string) (processHandle, Endpoint, error) {
	q, ok := qualified[id]
	if !ok || id == "magpie-tts" {
