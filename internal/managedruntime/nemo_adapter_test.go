...
func managedAdapterFixture(t *testing.T, root string) (*nemoAdapter, *[]string) {
	...
	t.Cleanup(srv.Close)
	pin := artifact.Asset{Backend: "cpu", URL: srv.URL, Size: int64(len(data)), SHA256: fmt.Sprintf("%x", sha256.Sum256(data))}
	recipe.Archives = []artifact.Asset{pin}
	...
	if err := artifact.Install(ctx, root, recipe.Bundle, srv.Client(), nil); err != nil {
		t.Fatal(err)
	}
...
func TestAdapterMissingInstallAndIsolation(t *testing.T) {
	...
	a := newAdapter(root)
	if _, _, err := a.Inspect(t.Context()); !errors.Is(err, os.ErrNotExist) {
	...
	if err := a.RemoveModel(t.Context(), "../foreign"); err == nil {
	...
	if err := a.RemoveModel(t.Context(), "nemotron-3.5"); err != nil {
		t.Fatal(err)
	}
