...
func TestArchiveRejectsUnsafePaths(t *testing.T) {
	for _, name := range []string{"../escape", "bin/../../escape", "/absolute", "C:/outside", "bin/evil:stream", "bin/CON", "bin/trailing. ", "bin\\..\\evil", "bin/../escape"} {
		t.Run(name, func(t *testing.T) {
			...
			os.Mkdir(dest, 0700)
			if extractArchive(t.Context(), z, dest) == nil {
	...
	if extractArchive(t.Context(), path, root) == nil {
...
func TestArchiveRejectsBomb(t *testing.T) {
	...
	if extractArchive(t.Context(), p, root) == nil {
...
func TestVerifiedAtomicInstall(t *testing.T) {
	...
	root := t.TempDir()
	if err := installTestAsset(t.Context(), root, a, srv.Client(), nil); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(root, "runtime", "bin", "nemo-speech.exe")); err != nil {
		t.Fatal(err)
	}
	if err := Verify(t.Context(), filepath.Join(root, "runtime"), Bundle{Archives: []Asset{a}, Executable: "bin/nemo-speech.exe"}); err != nil {
		t.Fatal(err)
	}
	os.WriteFile(filepath.Join(root, "runtime", "bin", "nemo-speech.exe"), []byte("tampered"), 0600)
	if Verify(t.Context(), filepath.Join(root, "runtime"), Bundle{Archives: []Asset{a}, Executable: "bin/nemo-speech.exe"}) == nil {
		t.Fatal("accepted changed executable")
	}
	a.SHA256 = "bad"
	other := t.TempDir()
	if installTestAsset(t.Context(), other, a, srv.Client(), nil) == nil {
...
func TestRuntimeInstallRejectsUnverifiedDownloads(t *testing.T) {
	...
	installers := map[string]func(context.Context, string, Asset, *http.Client, func(float64)) error{"bundle": installTestAsset}
...
func installTestAsset(ctx context.Context, root string, a Asset, client *http.Client, progress func(float64)) error {
	return Install(ctx, root, Bundle{Archives: []Asset{a}, Executable: "bin/nemo-speech.exe"}, client, progress)
}
