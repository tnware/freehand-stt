...
func TestDarwinRuntimeFixture(t *testing.T) {
	...
	case "listener":
		...
		time.Sleep(time.Minute)
	case "parent":
		p, err := launchOwned(t.Context(), os.Args[0], []string{"-test.run=^TestDarwinRuntimeFixture$"}, "", []string{"FREEHAND_RUNTIME_FIXTURE=tree"})
		if err != nil {
			os.Exit(2)
		}
...
func awaitNativeNumber(t *testing.T, p *Process) int {
	t.Helper()
	deadline := time.Now().Add(5 * time.Second)
