...
test.beforeEach(async ({ page }) => {
  await page.goto(
    "/tests/browser/app/?view=workspace&theme=dark&file-streaming",
  );
...
test("the response preference survives tab changes and unsupported connections", async ({
  ...
  await toggle.click();
  await page
    .getByRole("button", { name: "Completed-only profile", exact: true })
    .click();
  await expect(toggle).toBeDisabled();
  await expect(toggle).not.toBeChecked();
  await expect(
    page.getByRole("button", { name: "Try streaming", exact: true }),
  ).toHaveCount(0);
  await page
    .getByRole("button", { name: "Streaming supported", exact: true })
    .click();
  await expect(toggle).toBeChecked();
  await expect(
    page.getByText("File requests: 0; streaming: false", { exact: true }),
  ).toBeVisible();
});
...
test("Try streaming selects text updates for the next explicit request", async ({
  ...
  await page
    ...
    .click();
  await page
    .getByRole("button", { name: "Try streaming", exact: true })
    .click();
  ...
  await expect(
    page.getByText("File requests: 1; streaming: true", { exact: true }),
  ).toBeVisible();
});
...
test("an unsupported connection submits completed mode without losing the preference", async ({
  ...
  await page.getByRole("button", { name: "Transcribe", exact: true }).click();
  await expect(
    page.getByText("File requests: 1; streaming: false", { exact: true }),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "Streaming supported", exact: true })
    .click();
...
test("file start responds immediately, blocks conflicting actions, and recovers after rejection", async ({
  ...
  await page.goto(
    "/tests/browser/app/?view=workspace&theme=dark&file-streaming&file-actions",
  );
  await page.getByRole("button", { name: "Audio file", exact: true }).click();
  await page
    .getByRole("button", { name: "Streaming supported", exact: true })
  ...
  await expect(
    page.getByRole("button", { name: "Change audio file", exact: true }),
  ).toBeDisabled();
  await expect(
    page.getByRole("button", {
      name: "Clear selected audio file",
      exact: true,
    }),
  ).toBeDisabled();
  ...
  await page.getByRole("button", { name: "Transcribe", exact: true }).click();
  await page
    .getByRole("button", { name: "Admit file start", exact: true })
    .click();
  await expect(
    page.getByRole("button", { name: "Cancel", exact: true }),
  ).toBeEnabled();
  await expect(
    page.getByText("File requests: 2; streaming: true", { exact: true }),
  ).toBeVisible();
});
