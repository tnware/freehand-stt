...
for (const width of [560, 1156]) {
  test(`mouse wheel scrolls history entries at ${width}px`, async ({
    page,
  }) => {
...
for (const width of [560, 1156]) {
  test(`history follows the newest transcript and preserves manual reading at ${width}px`, async ({
    ...
    await page.screenshot({
      path: info.outputPath(`history-latest-${width}.png`),
    });
    ...
    await expect(
      page.getByRole("region", { name: "Raw transcript", exact: true }),
    ).toHaveCount(0);
...
test("an unretained file result is fully readable above collapsed history", async ({
  page,
}) => {
  ...
  const text = result.getByRole("textbox", {
    name: "Audio file transcript",
    exact: true,
  });
