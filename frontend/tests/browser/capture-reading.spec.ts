...
test("capture duration freezes when transcription clears the recording timestamp", async ({
  ...
  await page.goto(
    "/tests/browser/app/?view=workspace&theme=dark&capture-clock",
  );
  await page
    .getByRole("button", { name: "Simulate recording", exact: true })
    .click();
  const timer = page.locator('[aria-label="Capture duration"]');
  await expect(timer).toHaveText("00:04");
  await page
    .getByRole("button", { name: "Simulate transcription", exact: true })
    .click();
  await expect(timer).toHaveText("00:04");
...
for (const width of [560, 1100]) {
  test(`jump to latest stays outside transcript text at ${width}px`, async ({
    page,
  }, info) => {
    ...
    await page.screenshot({
      path: info.outputPath(`transcript-jump-${width}.png`),
    });
    await jump.click();
