...
for (const width of [520, 1000]) {
  test(`footer connection details follow the task and stay keyboard accessible at ${width}px`, async ({
    ...
    await page.goto(
      "/tests/browser/app/?view=workspace&diagnostics=normal&theme=dark",
    );
    ...
    await expect(
      panel(page).getByText("HTTP 200", { exact: true }),
    ).not.toBeVisible();
    ...
    await page.screenshot({
      path: info.outputPath(`connection-panel-${width}.png`),
    });
    ...
    await expect(
      page
        .getByRole("status")
        .filter({ hasText: "Edit voice-fixture for voice" }),
    ).toHaveCount(1);
    await page
...
test("a failed check can be retried from the footer", async ({
  page,
}, info) => {
  await page.goto(
    "/tests/browser/app/?view=workspace&diagnostics=failure&theme=dark",
  );
  await status(page).click();
  await panel(page)
    .getByRole("button", { name: "Check connection", exact: true })
...
test("draft checks are not displayed as checks of the applied connection", async ({
  page,
}) => {
...
test("disabled speech and missing connections offer setup without probing", async ({
  page,
}) => {
  ...
  await expect(panel(page).getByRole("button", { name: /Check/ })).toHaveCount(
    0,
  );
  await panel(page)
  ...
  await panel(page)
    .getByRole("button", { name: "Choose connection", exact: true })
    .click();
  await expect(
