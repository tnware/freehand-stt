...
for (const width of [560, 1000]) {
  test(`speech failures stay local and retry keeps the draft at ${width}px`, async ({
    ...
    await page.screenshot({
      path: info.outputPath(`speech-error-${width}.png`),
    });
    ...
    await dialog
      ...
      .click();
    await expect(
      page.locator("footer").filter({ hasText: "Speech settings" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("button", { name: "Save generated speech" }),
    ).toBeVisible();
    ...
    await page
      ...
      .click();
    await expect(
      page.getByRole("dialog", { name: "Error details", exact: true }),
    ).toContainText("Choose another folder");
    await page.keyboard.press("Escape");
...
test("file errors keep details and settings beside the existing retry", async ({
  page,
}) => {
  ...
  await details
    ...
    .click();
  await expect(
    page.locator("footer").filter({ hasText: "File transcription settings" }),
  ).toBeVisible();
...
test("a settings save failure has one owner while the decision dialog is open", async ({
  ...
  await expect(decision.getByRole("alert")).toContainText(
    "Fixture save failed",
  );
  await expect(
    page.getByRole("complementary", { name: "Notifications" }),
...
test("long dictation errors can be read by keyboard without enlarging the transport", async ({
  ...
  await page.setViewportSize({ width: 860, height: 660 });
  await page.goto(
    "/tests/browser/app/?view=workspace&feedback=voice&theme=dark",
  );
  ...
  await expect(
    page.getByText("No transcript to show", { exact: true }),
  ).toBeVisible();
  const details = page.getByRole("button", {
    name: "Dictation error details",
    exact: true,
  });
  await details.focus();
  await details.press("Enter");
  await expect(
    page.getByText(/The transcription request timed out\./),
  ).toHaveCount(1);
  await expect(
