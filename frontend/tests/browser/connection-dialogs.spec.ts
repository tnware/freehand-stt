...
for (const gesture of ["Escape", "Done", "titlebar"] as const) {
  async function close(page: Page) {
    if (gesture === "Escape")
      await page.locator("#connection-name").press("Escape");
    else if (gesture === "titlebar") await requestNativeClose(page);
    else await page.getByRole("button", { name: "Done", exact: true }).click();
  }
  test(`${gesture}: keep editing, discard, and reopen clear only the draft`, async ({
    page,
  }) => {
    await add(page);
    ...
    if (gesture === "titlebar")
      await page.evaluate(() =>
        window.testConnectionWindows.openSettings("server"),
      );
    await selected(page);
    await add(page);
    await expect(page.locator("#connection-name")).toHaveValue("");
  ...
  test(`${gesture}: pending save keeps errors and retry, then returns selection`, async ({
    page,
    saves,
  }) => {
    await add(page);
...
test("external hide clears pending prompt and credential draft before reopening the same shell", async ({
  page,
}) => {
...
test("reveal cannot replace an already-owned inline draft", async ({
  page,
}) => {
  await add(page);
...
test("dirty settings save before adding retains failure and retries without losing edits", async ({
  page,
  saves,
}) => {
  await openSection(page, "server");
