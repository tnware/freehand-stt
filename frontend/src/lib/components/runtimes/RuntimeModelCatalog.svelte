<script lang="ts">
  ...
  const families = $derived(runtimeModelFamilies(entry.id, models));
...
>
  {#each visibleFamilies as family (family.id)}
    {#if family.label}
      ...
      >
        <ChevronRightIcon
          class="size-3.5 shrink-0 text-muted-foreground {isFamilyOpen(family)
            ? 'rotate-90'
            : ''}"
          aria-hidden="true"
        />
    ...
    <div id={familyID(family.id)} hidden={!isFamilyOpen(family)}>
      {#each family.models as model (model.id)}
        {@const selected = model.id === row?.instance.model}
            ...
            <span class="model-actions flex items-center justify-end gap-1">
              ...
              {:else}
                ...
                {#if model.installed}<Button
                    ...
                    ><Trash2Icon class="size-3.5" aria-hidden="true" /></Button
                  >{/if}
              {/if}
            </span>
          </div>
