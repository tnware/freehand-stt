<script lang="ts">
  ...
  function updateFileTimeoutMinutes(event: Event) {
    ...
    if (Number.isFinite(n))
      settings.fileTranscriptionTimeoutSeconds = Math.round(n * 60);
  }
</script>
...
<SettingsCard>
  ...
  {:else}<RuntimeModelPicker
    ...
    <ModelProfilePicker
      ...
      onChange={(id) => {
        ...
        const languages = settings.modelProfiles.transcription?.find(
          (p) => p.id === id,
        )?.languages;
        if (
          languages?.length &&
  ...
  {#if settings.postProcessing.enabled && settings.postProcessing.preset === PostProcessingPreset.PostProcessingPresetS1Mini}
    <p class="px-5 pb-3 text-xs leading-relaxed text-muted-foreground">
      S1-mini cleanup is English only. Non-English selections or detected
      results keep the raw transcript. When no language is known, S1-mini runs
      assuming English. Choose custom cleanup or turn cleanup off for other
      languages.
    </p>
  {/if}
</SettingsCard>
