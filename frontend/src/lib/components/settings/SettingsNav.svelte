...
<SidebarContribution id="settings">
  <nav
  ...
  >
        ...
        <div class="flex shrink-0 flex-col gap-0.5 px-1.5">
          ...
          {#each sections as section (section.id)}
            ...
            >
              ...
              <span
                class="truncate {workbench
                  ? 'inline'
                  : 'hidden min-[760px]:inline'}">{section.label}</span
              >
              ...
              {#if section.id === "connections"}<ExternalLinkIcon
                  class="size-3 text-muted-foreground {workbench
                    ? 'block'
                    : 'hidden min-[760px]:block'}"
                />{/if}
    ...
    {#if onOpenChain}
      <div
        class="mt-auto px-1.5 pt-4 {workbench
          ? 'block'
          : 'hidden min-[760px]:block'}"
      >
