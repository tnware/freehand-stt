<script lang="ts">
  import NeMoTranscriptionControls from "./NeMoTranscriptionControls.svelte";
...
{#if capabilities?.nemoTranscriptionControls}
  <NeMoTranscriptionControls
    options={options.nemo}
    {managed}
    onChange={(nemo) => (options.nemo = nemo)}
  />
{:else}
  <SettingsDisclosure
  ...
  >
    <div class="px-5 py-4">
      <p class="text-xs leading-relaxed text-muted-foreground">
        Give the model context for recognizing speech. Shared terms are managed
        in Vocabulary.
      </p>
