<script lang="ts">
  ...
  import {
    ...
    type Status as UpdateStatus,
  } from "$bindings/updates";
  ...
  const platform = $derived(build?.platform || settings?.platform);
  const credentialStore = $derived(
    platform === "darwin" || platform === "windows"
      ? platformPresentation(platform).credentialStore
  ...
  let checkingForUpdates = $derived(
    updateStatus?.state === UpdateState.StateChecking,
  );
  ...
  onMount(() => {
    ...
    const offUpdates = Events.On(
      "updates:status",
      (event: { data: UpdateStatus }) => {
...
>
  <main class="min-h-0 flex-1 overflow-y-auto p-5" aria-label="About Freehand">
    <div class="mx-auto flex max-w-[560px] flex-col gap-4">
      ...
      <SettingsCard>
        ...
        <div class="border-t border-hairline px-5 py-3">
          ...
          <p class="mt-1 text-xs leading-relaxed text-muted-foreground">
            No Freehand subscription or account. Your chosen provider or hosting
        ...
        <div class="flex items-center gap-4 border-t border-hairline px-5 py-3">
          ...
          <Button
            ...
            disabled={checkingForUpdates ||
              updateStatus?.state === UpdateState.StateDevelopment}
            onclick={() => void checkForUpdates()}
          >
            <RefreshCw
              class={checkingForUpdates ? "size-3.5 animate-spin" : "size-3.5"}
            />
            Check now
          </Button>
        </div>
      ...
      <div
        class="flex flex-col gap-3 px-1 text-[13px] leading-relaxed text-muted-foreground"
      >
        <p>
        ...
        <p>
          ...
          screen. Recorded audio is deleted after every request, including failures
          and cancellation.
        </p>
      </div>
      <details
